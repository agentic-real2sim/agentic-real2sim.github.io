...
function setFilenameText(__filename) {
  ...
  // Display the base name with all extensions stripped (everything from the
  // first dot). Converter variants differ only in their extension chain
  // (e.g. "Foo.glb.blender.usdz" vs "Foo.glb.openusd-adobe-gltf.usdz"), so
  // showing just "Foo" keeps the panel width stable when switching converters.
  const _dot = _filename.indexOf('.');
  const _display = _dot > 0 ? _filename.slice(0, _dot) : _filename;
  /** @type {HTMLElement | null} */
  const _el = document.querySelector(".filename-text");
  if (_el) _el.innerText = _display;
  // Keep the full name for analytics + export naming.
  currentDisplayFilename = _filename;
}

// User-editable lighting config, provided by viewer-config.js as
// window.USD_VIEWER_LIGHTING. Missing fields fall back to sensible defaults.
function lightingConfig() {
  const c = (typeof window !== "undefined" && window.USD_VIEWER_LIGHTING) || {};
  return {
...
function resolveToneMapping(name) {
  switch (String(name || "agx").toLowerCase()) {
    case "none":
    case "off":
    case "linear":
...
// Applies the configured tone-mapping operator and exposure to every renderer
// we hand it (the main three.js renderer and the Hydra context renderer).
function applyAgXToneMapping(rendererLike) {
  const cfg = lightingConfig();
  const toneMapping = resolveToneMapping(cfg.toneMapping);
  const candidates = [
    rendererLike,
  ...
  for (const renderer of candidates) {
    ...
    if ("toneMappingExposure" in renderer) renderer.toneMappingExposure = cfg.exposure;
  }
}
...
function environmentUrlParam() {
  // Precedence: ?environment / ?env URL param > viewer-config.js > init option.
  return params.get("environment") || params.get("env")
    || lightingConfig().environment || options.hdrPath || DEFAULT_ENVIRONMENT;
}
...
try {
  Promise.all([getUsdModule({
    // Resolve the OpenUSD wasm bundle relative to this page so the viewer works
    // as a self-contained static folder (origin root OR a GitHub Pages subpath),
    // rather than the origin-root "/usd/" mount used by the dev server.
    mainScriptUrlOrBlob: new URL("./usd/bindings/emHdBindings.js", document.baseURI).href,
    locateFile: (file) => {
      return new URL("./usd/bindings/" + file, document.baseURI).href;
    },
    printErr: (...args) => {
...
async function init() {
  ...
  // viewer-config.js (window.USD_VIEWER_LIGHTING).
  {
    ...
    if (lc.ambient?.enabled) {
      const ambient = new AmbientLight(
        new Color(lc.ambient.color || "#ffffff"),
        typeof lc.ambient.intensity === "number" ? lc.ambient.intensity : 0.4
      );
    ...
    if (lc.keyLight?.enabled) {
      const key = new DirectionalLight(
        ...
        typeof lc.keyLight.intensity === "number" ? lc.keyLight.intensity : 2.0
      );
  ...
  */
  ...
  // Background, driven by viewer-config.js: null = transparent (page shows
  // through), "environment" = show the HDR image, or any hex color.
  {
    const bg = lightingConfig().background;
    if (typeof bg === "string" && bg.toLowerCase() === "environment") {
      envMapPromise.then(() => { scene.background = scene.environment; }).catch(() => {});
    } else if (typeof bg === "string" && bg.trim()) {
      scene.background = new Color(bg);
    } // else: leave transparent (default)
  }
  ...
  const converterIconSvg = {
    ...
    blender: '<svg class="conv-logo conv-logo-blender notranslate" translate="no" viewBox="0 0 24 24" fill="#E87D0D" aria-hidden="true"><path d="M12.51 13.214c.046-.8.438-1.506 1.03-2.006a3.424 3.424 0 0 1 2.212-.79c.85 0 1.631.3 2.211.79.592.5.983 1.206 1.028 2.005.045.823-.285 1.586-.865 2.153a3.389 3.389 0 0 1-2.374.938 3.393 3.393 0 0 1-2.376-.938c-.58-.567-.91-1.33-.865-2.152M7.35 14.831c.006.314.106.922.256 1.398a7.372 7.372 0 0 0 1.593 2.757 8.227 8.227 0 0 0 2.787 2.001 8.947 8.947 0 0 0 3.66.76 8.964 8.964 0 0 0 3.657-.772 8.285 8.285 0 0 0 2.785-2.01 7.428 7.428 0 0 0 1.592-2.762 6.964 6.964 0 0 0 .25-3.074 7.123 7.123 0 0 0-1.016-2.779 7.764 7.764 0 0 0-1.852-2.043h.002L13.566 2.55l-.02-.015c-.492-.378-1.319-.376-1.86.002-.547.382-.609 1.015-.123 1.415l-.001.001 3.126 2.543-9.53.01h-.013c-.788.001-1.545.518-1.695 1.172-.154.665.38 1.217 1.2 1.22V8.9l4.83-.01-8.62 6.617-.034.025c-.813.622-1.075 1.658-.563 2.313.52.667 1.625.668 2.447.004L7.414 14s-.069.52-.063.831zm12.09 1.741c-.97.988-2.326 1.548-3.795 1.55-1.47.004-2.827-.552-3.797-1.538a4.51 4.51 0 0 1-1.036-1.622 4.282 4.282 0 0 1 .282-3.519 4.702 4.702 0 0 1 1.153-1.371c.942-.768 2.141-1.183 3.396-1.185 1.256-.002 2.455.41 3.398 1.175.48.391.87.854 1.152 1.367a4.28 4.28 0 0 1 .522 1.706 4.236 4.236 0 0 1-.239 1.811 4.54 4.54 0 0 1-1.035 1.626"/></svg>',
    ...
    // Adobe corporate mark (brand red) for the Adobe glTF converter.
    adobe: '<svg class="conv-logo conv-logo-adobe notranslate" translate="no" viewBox="0 0 24 24" fill="#FF0000" aria-hidden="true"><path d="M13.966 22.624l-1.69-4.281H8.122l3.892-9.144 5.662 13.425zM8.884 1.376H0v21.248zm15.116 0h-8.86L24 22.624Z"/></svg>',
  ...
  // Bridge for the embedding page: switch USD files in-place (no full page /
  // WASM reload) by posting { type: 'usd-viewer:load', file, label }. This reuses
  // the exact code path as the built-in sample menu, so the previous stage is
  // cleared and the camera refit automatically.
  window.usdViewerLoadFile = function(fileUrl, label) {
    // Wait for the OpenUSD runtime before loading, so switch requests that
    // arrive before init finishes are queued rather than dropped.
    return usdModuleReadyPromise.then(() => loadFromFileLink({
      href: '?file=' + encodeURIComponent(fileUrl),
      dataset: { name: label || '' },
      textContent: label || '',
    }));
  };
  window.addEventListener('message', function(event) {
    ...
    if (data && data.type === 'usd-viewer:load' && typeof data.file === 'string') {
      window.usdViewerLoadFile(data.file, data.label);
    }
  });
  // Announce to the embedding page that the switch bridge is live, so it can
  // flush any file it queued before this iframe finished loading.
  function announceReady() {
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'usd-viewer:ready' }, '*');
      }
